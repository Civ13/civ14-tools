...
class ClothingIndexer {
	...
	parseClothings(jsonData) {
		...
		jsonData.Types.forEach((clothing) => {
			if (clothing.Path.startsWith("/obj/item/clothing/head/")) {
	...
	saveToYaml() {
		...
		for (const clothingName in this.index) {
			...
			let uniqueId = "civ13_head_" + parsedName;
			let count = 1;
		...
		fs.writeFileSync(
			"./../output/yml/entities_clothing_hats.yml",
			yamlStr,
			"utf8"
		);
		fs.writeFileSync(
			"./../output/yml/recipes_clothing_hats.yml",
			yamlRecp,
			"utf8"
		);
