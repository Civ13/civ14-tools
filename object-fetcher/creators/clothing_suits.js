...
class ClothingIndexer {
	...
	parseClothings(jsonData) {
		...
		jsonData.Types.forEach((clothing) => {
			if (clothing.Path.startsWith("/obj/item/clothing/suit/")) {
	...
	saveToYaml() {
		...
		for (const clothingName in this.index) {
			...
			let uniqueId = "civ13_suit_" + parsedName;
			let count = 1;
		...
		fs.writeFileSync(
			"./../output/yml/entities_clothing_suit.yml",
			yamlStr,
			"utf8"
		);
		fs.writeFileSync(
			"./../output/yml/recipes_clothing_suit.yml",
			yamlRecp,
			"utf8"
		);
