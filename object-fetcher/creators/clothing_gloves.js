...
class ClothingIndexer {
	...
	parseClothings(jsonData) {
		...
		jsonData.Types.forEach((clothing) => {
			if (clothing.Path.startsWith("/obj/item/clothing/gloves/")) {
	...
	saveToYaml() {
		...
		for (const clothingName in this.index) {
			...
			let uniqueId = "civ13_gloves_" + parsedName;
			let count = 1;
			...
			let recipe = newRecipe(
				...
				"Civ14/Clothing/exported/gloves/" + iconState + ".rsi",
				"icon",
				"Item"
			);
		...
		fs.writeFileSync(
			"./../output/yml/entities_clothing_gloves.yml",
			yamlStr,
			"utf8"
		);
		fs.writeFileSync(
			"./../output/yml/recipes_clothing_gloves.yml",
			yamlRecp,
			"utf8"
		);
