...
class ClothingIndexer {
	...
	parseClothings(jsonData) {
		...
		jsonData.Types.forEach((clothing) => {
			if (clothing.Path.startsWith("/obj/item/clothing/glasses/")) {
	...
	saveToYaml() {
		...
		for (const clothingName in this.index) {
			...
			let uniqueId = "civ13_glasses_" + parsedName;
			let count = 1;
			...
			let recipe = newRecipe(
				...
				"Civ14/Clothing/exported/glasses/" + iconState + ".rsi",
				"icon",
				"Item"
			);
		...
		fs.writeFileSync(
			"./../output/yml/entities_clothing_glasses.yml",
			yamlStr,
			"utf8"
		);
		fs.writeFileSync(
			"./../output/yml/recipes_clothing_glasses.yml",
			yamlRecp,
			"utf8"
		);
