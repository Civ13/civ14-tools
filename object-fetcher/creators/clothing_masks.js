...
class ClothingIndexer {
	...
	parseClothings(jsonData) {
		...
		jsonData.Types.forEach((clothing) => {
			if (clothing.Path.startsWith("/obj/item/clothing/mask/")) {
				if (
					!clothing.Path.startsWith(
	...
	saveToYaml() {
		...
		for (const clothingName in this.index) {
			...
			let uniqueId = "civ13_mask_" + parsedName;
			let count = 1;
			...
			let recipe = newRecipe(
				...
				"Civ14/Clothing/exported/masks/" + iconState + ".rsi",
				"icon",
				"Item"
			);
			...
			let newparent = "ClothingMaskBase";
			if (clothing.Path.startsWith("/obj/item/clothing/mask/gas/")) {
				newparent = "ClothingMaskGas";
			}
			if (
				!clothing.Path.startsWith("/obj/item/clothing/mask/smokable/")
			) {
				yamlStr += yaml.dump(
					convertToSS14(
						clothing.Variables.name,
						"civ13_mask_" + parsedName,
						clothing.Variables.desc,
						"Civ14/Clothing/exported/masks/" + iconState + ".rsi",
		...
		fs.writeFileSync(
			"./../output/yml/entities_clothing_masks.yml",
			yamlStr,
			"utf8"
		);
		fs.writeFileSync(
			"./../output/yml/recipes_clothing_masks.yml",
			yamlRecp,
			"utf8"
		);
