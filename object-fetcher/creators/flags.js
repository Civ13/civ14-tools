...
class ItemIndexer {
	...
	parseItem(jsonData) {
		...
		jsonData.Types.forEach((item) => {
			if (item.Path.startsWith("/obj/structure/flag/")) {
	...
	saveToYaml(filepath) {
		...
		for (const itemName in this.index) {
			...
			let parsedName = item.Path.replace("/obj/structure/flag/", "");
			parsedName = parsedName.replace("/", "_");
			console.log(parsedName);
			yamlStr += yaml.dump(
				convertToSS14("civ13_flag_" + parsedName, item.Variables)
			);
		}
...
indexer.saveToYaml("./../output/yml/flags.yml");
